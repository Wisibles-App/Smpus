...
const StartExam: React.FC = () => {
    ...
    const selectAnswer = ((selectedQuestion: any, answer: any) => {
        ...
        var found = finalReqObj.some((el: any) => el.onlineexam_question_id === selectedQuestion.id);
        if (!found) {
            selectedObj.onlineexam_question_id = selectedQuestion.id;
            selectedObj.select_option = answer;
            selectedObj.question_type = '';
            selectedObj.onlineexam_student_id = videoQuestionsList.exam.onlineexam_student_id;
            finalReqObj.push(selectedObj);
        }
        else {
            finalReqObj.forEach((element: any, index: any) => {
                if (element.onlineexam_question_id === selectedQuestion.id) {
   ...
   /* const sumitQuestions = ()=>{
        ...
        return api.post('/api/onlineexam/saveonlineexam', finalReqObj, {
            headers: {
    ...
    const sumitQuestions = () => {        
        ...
        if (finalReqObj.length === 0) {
            ...
            return api.post('/api_new/onlineexam/saveonlineexam', finalReqObj, {
                headers: {
                    "Content-Type": "application/json"
                }
            })
            ...
            presentAlert({
                header: 'Are you sure, you want to submit this exam?',
                buttons: [
                    {
                        text: 'Cancel',
                        role: 'cancel',
                    ...
                    },
                    {
                        ...
                        handler: () => {
                            ...
                            showIonLoading('Loading....');
                            api.post('/api_new/onlineexam/saveonlineexam', finalReqObj, {
                                headers: {
                                    "Content-Type": "application/json"
                                }
                            })
    ...
    const getOnlineVideoExamsQuestions = () => {
        ...
        const reqObj = {
            ...
            online_exam_id: history.location.state.onlineExamID.id
        };
        return api.post('api_new/Webservice/getOnlineExamQuestion', reqObj, {
            headers: {
                "Content-Type": "application/json",
    ...
    return (
        <IonPage>
            ...
            <IonButton onClick={handleBackButton} color="primary"> {/* Add color and other attributes */}
          ...
          <IonTitle >{history.location.state.onlineExamID.exam}</IonTitle>
            ...
            <IonContent>
                ...
                {videoQuestionsList?.exam?.questions?.map((question: any, index: number) => {
