...
const StartCetExam: React.FC = () => {
    ...
    const selectAnswer = ((selectedQuestion: any, answer: any) => {
        ...
        var found = finalReqObj.some((el: any) => el.onlineexamcet_question_id === selectedQuestion.id);
        if (!found) {
            selectedObj.onlineexamcet_question_id = selectedQuestion.id;
            selectedObj.select_option = answer;
            selectedObj.question_type = '';
            selectedObj.onlineexamcet_student_id = videoQuestionsList.exam.onlineexamcet_student_id;
            finalReqObj.push(selectedObj);
        }
        else {
            finalReqObj.forEach((element: any, index: any) => {
                if (element.onlineexamcet_question_id === selectedQuestion.id) {
   ...
   /* const sumitQuestions = ()=>{
    ...
    const sumitQuestions = () => {        
        ...
        if (finalReqObj.length === 0) {
            ...
            return api.post('/api_new/onlineexamcet/saveonlineexamcet', finalReqObj, {
                headers: {
                    "Content-Type": "application/json",
                }
            })
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        handler: () => {
                            ...
                            api.post('/api_new/onlineexamcet/saveonlineexamcet', finalReqObj, {
                                headers: {
                                    "Content-Type": "application/json",
                                }
                            })
    ...
    const getOnlineVideoExamsQuestions = () => {
        ...
        return api.post('api_new/Webservice/getOnlineExamCetQuestion', reqObj, {
            headers: {
                "Content-Type": "application/json",
